...
API & Realtime Gateway
   ↓
Command Service ─→ KV (权威状态) ─→ MQ (唤醒/事件)
                    │                 │
...
> 说明:Phase 1 启动后,代码骨架将落到 `src/{domain,application,infrastructure,interfaces,bootstrap}` 下(参见 arch.md §15)。

## 路线图

- **Phase 1 — 最小可恢复闭环**:Chat/Run/Machine Definition IR/`onState` 内核/LLM+Tool Handler/Outbox/SSE/对象产物/取消与基础重试。
- **Phase 2 — 生产可用**:多租户 IAM、MCP/A2A/RAG、远程 Handler、长期 Memory、DLQ、限流熔断降级。
